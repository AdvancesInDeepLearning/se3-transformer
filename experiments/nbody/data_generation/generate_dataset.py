...
import numpy as np
from synthetic_sim import ArgonSim, ChargedParticlesSim, SpringSim
...
parser.add_argument(
    "--name", type=str, default="new", help="Add string to suffix of filename."
)
parser.add_argument(
    "--num-train",
    type=int,
...
parser.add_argument(
    "--dim", type=int, default=3, help="Spatial simulation dimension (2 or 3)."
)
parser.add_argument(
    "--boxsize",
    type=float,
...
if args.simulation == "springs":
    sim = SpringSim(
        noise_var=0.0, n_balls=args.n_balls, box_size=args.boxsize, dim=args.dim
    )
...
ds["train"] = generate_dataset(args.num_train, args.length, args.sample_freq)
ds["train"]["git_commit"] = str(git_commit)
ds["train"]["args"] = args_dict

print("Generating {} test simulations".format(args.num_test))
ds["test"] = generate_dataset(args.num_test, args.length_test, args.sample_freq)
