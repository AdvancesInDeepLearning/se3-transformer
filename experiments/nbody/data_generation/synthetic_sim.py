...
class ArgonSim(object):
    def __init__(
        ...
        self.epsilon_kb = 1.0 - 0  # another natural constant
    ...
    def _difference_matrix(self, x0, x1):
        ...
        if self.verbose:
            print("delta", np.stack(delta, axis=0).shape)
    ...
    def get_gradient(self, r_vec):
        ...
        if self.verbose:
            print("grad_lennard_jones", grad_lennard_jones.shape)
    ...
    def get_force(self, state):
        ...
        differences = self._difference_matrix(state, state)
        ...
        if self.verbose:
            print("forces", np.sum(self.get_gradient(differences), axis=-1))
    ...
    def sample_trajectory(self, T=10000, sample_freq=10, charge_prob=[1.0]):
        n = self.n_balls
        ...
        vel_next = np.random.randn(
            self.dim, n
        )  # implement boltzmann velocity profile here ? Maybe not needed if we use Rescaling.
        clamp_next = np.zeros(
            [
                n,
            ]
...
class InitializeFCC:
    ...
    def __call__(self):
        ...
        for x in np.linspace(0, self.boxsize, self.number_of_cells, endpoint=False):
            for y in np.linspace(0, self.boxsize, self.number_of_cells, endpoint=False):
                for z in np.linspace(
                    0, self.boxsize, self.number_of_cells, endpoint=False
                ):
        ...
        new_arr += np.random.uniform(
            -self.noise_level, self.noise_level, size=(new_arr.shape)
        )
        return new_arr
...
if __name__ == "__main__":
    ...
    t = time.time()
    loc, vel, edges, charges, clamp = sim.sample_trajectory(T=5000, sample_freq=100)
