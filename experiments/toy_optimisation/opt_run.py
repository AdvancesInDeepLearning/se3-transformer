...
def train_epoch(epoch, model, loss_function, dataloader, optimizer, schedule, FLAGS):
    ...
    gradient_descent_model = GradientDescent(
    ...
    for i, g in enumerate(dataloader):
        ...
        if "update_norm_0" in pred.ndata and FLAGS.num_iter > 1:
            for j in range(FLAGS.num_iter):
                update_norm_steps_log[j].append(
                    np.mean(to_np(pred.ndata[f"update_norm_{j}"]))
                )
        ...
        if FLAGS.gd_post_process:
            with torch.no_grad():
                ...
                tuned_update_norm_log.append(
                    np.mean(to_np(tuned_pred.ndata["update_norm"]))
                )
                ...
                if i % FLAGS.log_interval == 0:
                    wandb.log(
                        {"Train Batch Tuned Loss": to_np(tuned_loss)}, commit=False
                    )
    ...
    if FLAGS.gd_post_process:
        ...
        wandb.log(
            {"Average Tuned Update Norm": average_tuned_update_norm}, commit=False
        )
...
def main(FLAGS, UNPARSED_ARGV):
    ...
    model = models.__dict__.get(FLAGS.model)(
        ...
        compute_gradients=FLAGS.basis_gradients,
        k_neighbors=FLAGS.k_neighbors,
        step_size=FLAGS.gd_stepsize,
    ...
    utils_logging.write_info_file(
        model, FLAGS=FLAGS, UNPARSED_ARGV=UNPARSED_ARGV, wandb_log_dir=wandb.run.dir
    )
    ...
    if FLAGS.forward or isinstance(model, GradientDescent):
        ...
        for epoch in range(FLAGS.num_epochs):
            ...
            train_epoch(
                epoch, model, task_loss, dataloader, optimizer, scheduler, FLAGS
            )

    if FLAGS.log_steps:
        log_dataset = OptDataset(FLAGS, split="log")
        log_loader = DataLoader(
            log_dataset, batch_size=FLAGS.batch_size, shuffle=False, collate_fn=collate
        )
...
for run in range(FLAGS.num_runs):
    try:
        ...
        with wandb.init(
            project="iterativeSE3",
            name=f"{FLAGS.name}{run:02d}",
            config=FLAGS,
            reinit=True,
        ):
            wandb.save("*.txt")
