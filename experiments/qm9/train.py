...
def train_epoch(epoch, model, loss_fnc, dataloader, optimizer, scheduler, FLAGS):
    ...
    for i, (g, y) in enumerate(dataloader):
        ...
        if i % FLAGS.log_interval == 0:
            wandb.log(
                {"Train L1 loss": to_np(l1_loss), "Rescale loss": to_np(rescale_loss)}
            )
...
def main(FLAGS, UNPARSED_ARGV):
    ...
    train_dataset = QM9Dataset(
        FLAGS.data_address, FLAGS.task, mode="train", transform=RandomRotation()
    )
    ...
    val_dataset = QM9Dataset(FLAGS.data_address, FLAGS.task, mode="valid")
    ...
    test_dataset = QM9Dataset(FLAGS.data_address, FLAGS.task, mode="test")
...
if __name__ == "__main__":
    ...
    # Model parameters
    parser.add_argument(
        "--model", type=str, default="SE3Transformer", help="String name of model"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--num_nlayers", type=int, default=0, help="Number of layers for nonlinearity"
    )
    parser.add_argument(
        "--fully_connected", action="store_true", help="Include global node in graph"
    )
    parser.add_argument(
        "--div", type=float, default=4, help="Low dimensional embedding fraction"
    )
    parser.add_argument(
        "--pooling", type=str, default="avg", help="Choose from avg or max"
    )
    ...
    parser.add_argument(
        "--wandb", type=str, default="equivariant-attention", help="wandb project name"
    )
